...
namespace FleetClients
{
    internal class FleetManagerClient : AbstractCallbackClient<IFleetManagerService>, IFleetManagerClient
    {
        ...
        /// <summary>
        /// Get the current state of the fleet's vehicles
        /// </summary>
        /// <param name="fleetState">The collection of vehicle states</param>
        /// <returns>ServiceOperationResult</returns>
        public ServiceOperationResult TryGetCurrentFleetState(out FleetState fleetState)
        {
            Logger.Info("TryGetCurrentFleetState()");

            try
            {
                var result = GetCurrentFleetState();
                fleetState = result.Item1;
        ...
        private Tuple<FleetState, ServiceCallData> GetCurrentFleetState()
        {
            Logger.Debug("GetCurrentFleetState()");

            if (isDisposed) throw new ObjectDisposedException("FleetManagerClient()");

            Tuple<FleetState, ServiceCallData> result;

            using (ChannelFactory<IFleetManagerService> channelFactory = CreateChannelFactory())
            {
                IFleetManagerService channel = channelFactory.CreateChannel();
                result = channel.GetCurrentFleetState();
                channelFactory.Close();
            }
